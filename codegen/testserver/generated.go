...
func (ec *executionContext) _Subscription_updated(ctx context.Context, field graphql.CollectedField) func() graphql.Marshaler {
	...
	// FIXME: subscriptions are missing request middleware stack https://github.com/HaswinVidanage/gqlgen/issues/259
	//          and Tracer stack
	rctx := ctx
	...
	for k, v := range asMap {
		...
		case "thirdParty":
			...
			if data, ok := tmp.(*ThirdParty); ok {
				...
				return it, fmt.Errorf(`unexpected type %T from directive, should be *github.com/HaswinVidanage/gqlgen/codegen/testserver.ThirdParty`, tmp)
			}
		}
	}
